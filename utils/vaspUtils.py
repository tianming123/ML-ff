...
class VaspData:
    def __init__(self, vasprun_file_path):
        ...
        self.energy_path = "modeling >calculation > energy > i[name='e_0_energy'] "
    ...
    def track_position(self):
        ...
        for elem in position_str:
            ...
            for e in position_info:
                tmp.append([float(item) for item in e.get_text().strip().split()])
            ret.append(tmp)
        self.position_list = ret

    def track_energy(self):
        energy_str = self.soup.select(self.energy_path)
        tmp = []
        for e in energy_str:
            tmp.append(float(e.get_text().strip()))
        self.track_energy_list = tmp
